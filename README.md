...
### 2. 器件管理（含存取数量）
- 名称/功能/编号/封装/分类/位置/备注多维度管理，每个器件记录库存数量
- 搜索（名称/功能/编号）+ 器件架筛选（A/B/C/D）
- 器件卡片右下角显示 `×N` 数量；数量为 0 显示红色"缺货"
- **点击卡片 → 亮灯定位 + 存取弹窗**：点亮对应位置灯引导目视定位，同时弹出底部存取弹窗（StockInOutSheet）
  - `+` / `-` 按钮步进或手动输入目标数量（0 ~ 99999）
  - 显示"当前库存 → 新库存"对比，实时提示"存入 X 件"（蓝色）/"取出 X 件"（绿色）
  - 确认后事务化更新数量，关闭弹窗自动灭灯；取消不改变数量
- **左滑**卡片调出"编辑"/"删除"按钮
...
### 5. 蓝牙连接（仅 W02_ 前缀设备）
- **只扫描 `W02_` 开头的蓝牙模块**（过滤其他蓝牙设备），不限制信号强度，所有 W02_ 设备按信号强度降序排列
...
### 2. 存取器件（存入/取出数量）
- **库存页点击器件卡片**：点亮该位置灯做视觉引导，同时弹出底部"存取弹窗"
- **调整数量**：弹窗内用 `+` / `-` 按钮步进，或直接点击数字输入目标数量（0 ~ 99999）
- **存入/取出**：目标 > 当前 → 存入（蓝色提示）；目标 < 当前 → 取出（绿色提示）
- **确认**：点击"确认"后数量事务化写入数据库，弹窗关闭并自动灭灯；取消/遮罩不改变数量
- **BOM 页**：点击绿色（已匹配）BOM 条目切换亮/灭灯（纯亮灯引导，不涉及数量变更）
...
| 存取弹窗 | StockInOutSheet | 存入/取出数量底部弹窗（+/-/手动输入） |
...
│   ├── src/
│   │   ├── components/               # UI 组件 (SwipeableRow, StockInOutSheet, ImageUploadField, ErrorBoundary)
│   │   ├── context/                  # React Context (UserContext)
│   │   ├── navigation/               # 导航 (AppNavigator)
│   │   ├── screens/                  # 9+1 个页面
│   │   │   ├── DeviceListScreen.js       # 库存（主屏，点击卡片 → 亮灯 + 存取弹窗）
...
### Q1: 蓝牙扫描不到设备？
本 App **只扫描 `W02_` 开头的蓝牙模块**，其他设备被过滤（不限制信号强度，所有 W02_ 设备都会显示，按信号强度排序）。如果你的模块名称不符，需在厂商工具里改名，或修改 `BluetoothHandler.js` 的 `startsWith('W02_')` 条件。
...
**新增功能：**
- **存取数量功能**：点击器件卡片不再直接切换亮/灭灯，改为亮灯定位 + 弹出存取弹窗（StockInOutSheet）
  - `+` / `-` 按钮步进调整目标数量，支持手动输入（0 ~ 99999）
  - 弹窗展示"当前库存 → 新库存"对比，实时提示"存入 X 件"（蓝色）/"取出 X 件"（绿色）
  - 确认后事务化写入数据库，关闭弹窗自动灭灯；取消/遮罩关闭不改变数量
  - 键盘遮挡适配（监听键盘高度，弹窗自动上移）
- **器件卡片显示数量**：右下角显示 `×N`，数量为 0 显示红色"缺货"

**调整：**
- 蓝牙扫描取消 RSSI 强度阈值限制（不再要求 > -80dBm），只要名称以 `W02_` 开头就显示，仍按信号强度降序排序
- ConnectionScreen 空设备提示文案同步更新
